...
const getChoiceValue = (choice) => {
	...
	if (isObject && choice.value != null) { return choice.value; }
	if (isObject && choice.name != null) { return choice.name; }
	if (isObject && choice.key != null) { return choice.key; }
	return choice;
};
...
const listTypeBypass = (v, prompt) => {
	...
	if (choice != null) { return getChoiceValue(choice); }
...
const typeBypass = {
	...
	checkbox(v, prompt) {
		...
		const valuesNoMatch = valList.filter(
			val => !prompt.choices.some(
				(c, idx) => choiceMatchesValue(c, idx, val)
			)
		);
		...
		return valList.map(
			val => getChoiceValue(prompt.choices.find(
				(c, idx) => choiceMatchesValue(c, idx, val)
			))
		);
	},
...
export default function (prompts, bypassArr, plop) {
	const noop = [prompts, {}, []];
	// bail out if we don't have prompts or bypass data
	if (!Array.isArray(prompts)) { return noop; }
